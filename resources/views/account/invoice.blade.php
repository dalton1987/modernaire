	...
	<hr>
        <div class="for-custom-width">
             ...
             <div class="row">
                    <div class="col-md-10">
                        <address>
                        @if($order->paypal_paymentID != '')
                            <strong>Transaction ID:</strong><br>
                            {{$order->paypal_paymentID}}<br>
                            <strong>Payer ID:</strong><br>
                            {{$order->paypal_payerID}}<br><br>
                            @endif
            ...
            <div class="row">
                <div class="col-md-2 text-right">
                    ...
                    <strong>Billing Address:</strong><br>
                        ...
                        {{$order->billing_address_1}} , {{$order->billing_city}}, {{$order->billing_state}}, {{$order->billing_country}}, {{$order->billing_zip_code}}
                    </address>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-2 text-right">
                    ...
                    {{$order->delivery_address_1}} , {{$order->delivery_city}}, {{$order->delivery_state}}, {{$order->delivery_country}}, {{$order-delivery_zip_code}}
    			...
    			<div class="panel-body">
    						...
    						<tbody>
    							...
    							@foreach($order_products as $order_product)
									<tr>
										...
										<td class="text-center">${{number_format($order_product->order_products_price,2)}}</td>
										<td class="text-center">{{$order_product->order_products_qty}}</td>
										<td class="text-right">${{number_format($order_product->order_products_subtotal, 2)}}</td>
									</tr>
								...
								<tr>
    								...
    								<td class="thick-line text-right">Free</td>
    				...
    				<!--			</tr>-->
    							...
    							<tr>
									...
									?>
    								<td class="no-line text-right">${{number_format($order->order_item_total  + $shipping,2) }}</td>
    							</tr>
    						</tbody>
    					</table>
    				</div>
